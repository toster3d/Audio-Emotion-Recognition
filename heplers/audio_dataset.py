...
class AudioDataset(Dataset):
    def __init__(self, data, max_length=48000*3, sample_rate=16000, augment=False):
        ...
        if self.augment:
            self.time_mask = torchaudio.transforms.TimeMasking(time_mask_param=int(max_length * 0.1))
    ...
    def __getitem__(self, idx):
        ...
        if audio_tensor.shape[0] > self.max_length:
            ...
            if self.augment:
                start = torch.randint(0, audio_tensor.shape[0] - self.max_length, (1,)).item()
        ...
        if self.augment and torch.rand(1).item() < 0.5:
            ...
            if len(audio_tensor.shape) == 1:
                audio_tensor = audio_tensor.unsqueeze(0)  # Dodaj wymiar kanału dla TimeMasking
        ...
        audio_tensor = audio_tensor.unsqueeze(0)  # Dodanie wymiaru kanału (mono = 1 kanał)
