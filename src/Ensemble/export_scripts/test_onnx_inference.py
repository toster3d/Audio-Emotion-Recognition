...
class ONNXModelTester:
    ...
    def _load_session(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Błąd ładowania modelu: {e}") from e
    ...
    def generate_test_data(
        self, batch_size: int = 1, time_steps: int = 300
    ) -> Dict[str, np.ndarray]:
    ...
    def run_inference(
        ...
        try:
            ...
            raise RuntimeError(f"Błąd inference: {e}") from e
    ...
    def benchmark_performance(
        ...
        for batch_size in batch_sizes:
            ...
            stats = {
                ...
                "latency_per_sample": np.mean(times_array)
                / batch_size
                * 1000,  # ms/próbka
            }
            ...
            print(
                f"   ✅ Średni czas: {stats['mean_time'] * 1000:.2f} ± {stats['std_time'] * 1000:.2f} ms"
            )
    ...
    def test_accuracy(
        self, test_data: Dict[str, np.ndarray], expected_shape: Optional[Tuple] = None
    ) -> Dict:
        ...
        results = {"inference_time": inference_time, "outputs": []}

        for i, (output_name, expected_output_shape) in enumerate(
            zip(
                self.output_shapes.keys(),
                [expected_shape] if expected_shape else [None],
            )
        ):
            output = outputs[i]
            ...
            if expected_output_shape and output.shape != expected_output_shape:
                output_info["shape_error"] = (
                    f"Oczekiwano {expected_output_shape}, otrzymano {output.shape}"
                )
            ...
            if len(output.shape) == 2 and output.shape[1] == len(CLASS_NAMES):
                ...
                output_info["predictions"] = {
                    ...
                    "predicted_emotions": [
                        CLASS_NAMES[idx] for idx in predicted_classes
                    ],
                }
            ...
            print(
                f"      Zakres: [{output_info['min_value']:.4f}, {output_info['max_value']:.4f}]"
            )
            print(
                f"      Średnia: {output_info['mean_value']:.4f} ± {output_info['std_value']:.4f}"
            )

            if "probability_check" in output_info:
                prob_check = output_info["probability_check"]
                print(
                    f"      Softmax: {prob_check['is_softmax']} (suma: {prob_check['prob_sum_mean']:.4f})"
                )
...
def compare_models(
    model_paths: List[str], batch_sizes: Optional[List[int]] = None, num_iterations: int = 50
) -> None:
    ...
    for batch_size in batch_sizes:
        ...
        print(
            f"{'Model':<25} {'Latency (ms)':<15} {'Throughput (sps)':<20} {'P95 (ms)':<12}"
        )
        print("-" * 72)
        ...
        for model_name, results in all_results.items():
            if results and batch_size in results:
                ...
                print(
                    f"{model_name:<25} {latency:<15.2f} {throughput:<20.1f} {p95:<12.2f}"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--model_dir", type=str, help="Katalog z modelami do porównania"
    )
    parser.add_argument(
        "--batch_sizes",
        nargs="+",
        type=int,
        default=[1, 4, 8],
        help="Rozmiary batch do testowania",
    )
    parser.add_argument(
        "--iterations", type=int, default=100, help="Liczba iteracji benchmarku"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Porównaj wszystkie modele w katalogu"
    )
    ...
    if args.compare and args.model_dir:
        ...
        try:
            ...
            performance_results = tester.benchmark_performance(
                batch_sizes=args.batch_sizes, num_iterations=args.iterations
            )
