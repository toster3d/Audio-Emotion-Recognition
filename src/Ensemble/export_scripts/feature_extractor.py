...
try:
    ...
    print(
        "Ostrzeżenie: Nie można zaimportować konfiguracji. Używam domyślnych wartości dla ekstrakcji cech."
    )
...
def extract_feature_single(
    ...
    # np. 'spectrogram', 'spectral_contrast', 'zcr', 'rms', 'tempogram', 'tonnetz', 'delta_mfcc', 'delta_tempogram'
        ...
        print(
            f"Ostrzeżenie: Nieznany lub nieobsługiwany typ cechy w tym kontekście: {feature_type}"
        )
        return None
...
def extract_features_for_model(
    ...
    for ft_type in feature_types:
        if ft_type not in feature_params_map:
            print(
                f"Ostrzeżenie: Typ cechy '{ft_type}' nie jest mapowany na znaną nazwę wejścia ONNX. Pomijam."
            )
            continue
        ...
        if current_feature is not None:
            ...
            if current_feature.shape[0] != num_expected_features:
                print(
                    f"Ostrzeżenie: Wyekstrahowana cecha {ft_type} ma "
                    f"{current_feature.shape[0]} wymiarów cech, oczekiwano "
                    f"{num_expected_features}. Sprawdź parametry."
                )
                # Można tu dodać padding/cropping lub rzucić błąd, jeśli to krytyczne
                # Na razie kontynuujemy, ale model może się wywalić

            reshaped_feature: np.ndarray = current_feature.reshape(
                1, 1, current_feature.shape[0], -1
            ).astype(np.float32)
            output_features[feature_params_map[ft_type]["onnx_input_name"]] = (
                reshaped_feature
            )
        else:
