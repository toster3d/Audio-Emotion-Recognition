...
from src.config import (
    VGG16_DURATION as DURATION,
)
from src.config import (
    VGG16_EMOTION_MAPPING as EMOTION_MAPPING,
)
from src.config import (
    VGG16_EMOTION_NAMES as EMOTION_NAMES,
)
from src.config import (
    VGG16_N_MELS as N_MELS,
)
from src.config import (
    VGG16_SAMPLE_RATE as SAMPLE_RATE,
)
...
def load_nemo_dataset():
    ...
    try:
        ...
        print(
            f"Przykłady testowe: {len(dataset.get('test', dataset.get('validation', [])))} (test/validation)"
        )
        ...
        for emotion, count in emotions.items():
            print(
                f"   - {emotion}: {count} próbek ({count / len(dataset['train']) * 100:.1f}%)"
            )

        return dataset
    except Exception as e:
        print(f"❌ Wystąpił błąd podczas ładowania datasetu: {e!s}")
        raise e
...
def prepare_data(dataset, for_torch=True):
    ...
    mel_spectrograms = []
    labels = []
    count_by_emotion = dict.fromkeys(EMOTION_MAPPING.keys(), 0)
    skipped_samples = 0
    ...
    for sample in tqdm(dataset["train"], desc="Przetwarzanie próbek"):
        try:
            ...
            skipped_samples += 1
            print(f"❌ Pominięto próbkę z powodu błędu: {e!s}")
...
def predict_emotion(model, audio_path, device=None):
    ...
    try:
        ...
        # Wyniki
        print("✅ Wyniki analizy emocji:")
        print(
            f"   Przewidziana emocja: {EMOTION_NAMES[predicted_class]} ({probabilities[predicted_class]:.2%})"
        )
        print("   Wszystkie prawdopodobieństwa:")

        # Sortowanie emocji według prawdopodobieństwa
        emotions_probs = [
            (EMOTION_NAMES[i], probabilities[i]) for i in range(len(EMOTION_NAMES))
        ]
        ...
        print(f"❌ Wystąpił błąd podczas analizy pliku audio: {e!s}")
        import traceback
