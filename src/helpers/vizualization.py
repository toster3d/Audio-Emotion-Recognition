...
def generate_accuracy_comparison_plot(results_df, save_dir):
    ...
    fig.update_layout(
        height=600, width=STANDARD_WIDTH, showlegend=False, template="plotly_white"
    )
...
def generate_emotion_visualizations(emotions_df, results_df, save_dir):
    ...
    pivot_df = emotions_df.pivot(
        index="Feature Type", columns="Emotion", values="F1-score"
    )
    ...
    # 2. Wykres słupkowy dla trzech najlepszych reprezentacji - porównanie F1-score
    top_path = None
    ...
    for i, feature in enumerate(feature_types):
        ...
        fig_radar.update_layout(
            **{f"polar{i + 1}": {"radialaxis": {"visible": True, "range": [0, 100]}}}
        )
    ...
    fig_dashboard = make_subplots(
        rows=3, cols=2, subplot_titles=emotions, vertical_spacing=0.1
    )
    ...
    for i, emotion in enumerate(emotions):
        ...
        fig_dashboard.update_yaxes(
            title_text="F1-score (%)", range=[0, 110], row=row, col=col
        )
