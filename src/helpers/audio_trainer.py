...
class ModelTrainer:
    def __init__(
        self, model, criterion, optimizer, device="cuda", output_dir="model_outputs"
    ):
        self.model = model
    ...
    def train_model(self, train_loader, val_loader, num_epochs=20, patience=5):
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                print(
                    f"Model saved with validation accuracy: {val_acc:.4f} to {model_path}"
                )
            else:
                patience_counter += 1
                if patience_counter >= patience:
                    print(
                        f"Early stopping at epoch {epoch + 1}. Best model was at epoch {best_epoch + 1}."
                    )
                    break
