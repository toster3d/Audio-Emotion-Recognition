...
from src.helpers.ensemble_dataset import EnsembleDatasetIndexed, ensemble_collate_fn
...
class EnsembleModelTrainer:
    ...
    def _load_base_models(self):
        ...
        for feature_type, model_path in self.model_paths.items():
            try:
                print(
                    f"Próba załadowania modelu dla cechy '{feature_type}' z {model_path}"
                )
                ...
                failed_count += 1
                print(
                    f"Wystąpił błąd podczas ładowania modelu dla cechy '{feature_type}': {e}"
                )
        ...
        if not self.base_models:
            ...
            print(
                "3. Czy klasa modelu (model_class) jest zgodna z architekturą zapisanych modeli"
            )
    ...
    def _create_dataset(self):
        ...
        if self.dataset is None:
            try:
                ...
                if "label_encoder" in data and hasattr(
                ...
                raise RuntimeError(f"Błąd podczas tworzenia zbioru danych: {e}") from e
    ...
    def optimize_weights(
        self, n_trials=None, timeout=None, n_folds=None, test_size=None
    ):
        """
        ...
        Argumenty:
            n_trials (int, optional): Liczba prób Optuna. Jeśli None, używana jest wartość z konfiguracji.
                        timeout (int, optional): Limit czasu optymalizacji w sekundach.
                Jeśli None, używana jest wartość z konfiguracji.
            n_folds (int, optional): Liczba foldów walidacji krzyżowej.
                Jeśli None, używana jest wartość z konfiguracji.
            test_size (float, optional): Frakcja danych testowych.
                Jeśli None, używana jest wartość z konfiguracji.
            ...
            with mlflow.start_run(run_name=f"trial_{trial.number}", nested=True):
                ...
                for i, (train_indices, val_indices) in enumerate(eval_folds):
                    ...
                    ensemble = WeightedEnsembleModel(
                        self.base_models, normalized_weights
                    ).to(self.device)
        ...
        with mlflow.start_run(run_name=f"optuna_optimization_{timestamp}"):
            ...
            best_weights = {
                k.replace("weight_", ""): v for k, v in study.best_params.items()
            }
            ...
            with open(
                os.path.join(self.output_dir, "optimization_results.json"), "w"
            ) as f:
    ...
    def train_and_evaluate(
        ...
        with mlflow.start_run():
            ...
            ensemble_model = WeightedEnsembleModel(self.base_models, weights).to(
                self.device
            )
            ...
            for _i, class_name in enumerate(self.class_names):
                if class_name in test_results["report"]:
                    ...
                    mlflow.log_metric(
                        f"test_{class_name}_f1-score", class_metrics["f1-score"]
                    )
            ...
            with open(
                os.path.join(self.output_dir, "evaluation_results.json"), "w"
            ) as f:
    ...
    def analyze_errors(
                    ...
                    errors.append(
                        {
                            ...
                            "true_label_confidence": torch.softmax(output, dim=1)[0][
                                label
                            ].item(),
                        }
                    )
        ...
        error_df["pred_class"] = error_df["pred_label"].apply(
            lambda x: self.class_names[x]
        )
