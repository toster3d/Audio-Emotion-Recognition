...
from src.config import CLASS_NAMES, CV_FOLDS, DEVICE, SEED
...
def load_pretrained_model(model_path, model_class, num_classes=6, device=None):
    ...
    for loading_strategy in [
        ...
        ),  # Strategia 1: Standardowe ładowanie
        lambda: torch.load(
            model_path, map_location=device, weights_only=False
        ),  # Strategia 2: Ładowanie z weights_only=False
    ]:
        try:
...
def evaluate_model(
    model, dataloader, device=None, class_names=None, return_probs=False
):
    """
    ...
    results = calculate_metrics(
        all_labels, all_preds, all_probs, class_names, return_probs
    )
    return results


def calculate_metrics(
    true_labels, predictions, probabilities=None, class_names=None, return_probs=False
):
    """
