...
def explain_with_lime(image, predict_fn, num_samples=5000, num_features=30):
    ...
    for i in range(segments_h):
        for j in range(segments_w):
            segments[
                i * h // segments_h : (i + 1) * h // segments_h,
                j * w // segments_w : (j + 1) * w // segments_w,
            ] = i * segments_w + j
    ...
    # importances nie jest używane, więc usunięte

    # Sortuj segmenty według ich wagi (absolutnej wartości)
    # sorted_importances nie jest używane, więc usunięte
...
def verify_lime_explanation(model, sample, mask, device, reverse_label_mapping):
    ...
    with torch.no_grad():
        ...
        original_probs = (
            torch.nn.functional.softmax(original_output, dim=1).cpu().numpy()[0]
        )
    ...
    binary_mask = mask[:, :, 0] > 0 if mask.ndim > 2 else mask > 0
    ...
    masked_tensor = (
        torch.tensor(masked_sample).unsqueeze(0).unsqueeze(0).float().to(device)
    )
    with torch.no_grad():
        masked_output = model(masked_tensor)
        masked_probs = (
            torch.nn.functional.softmax(masked_output, dim=1).cpu().numpy()[0]
        )
    ...
    if num_pixels > 0:
        random_indices = np.random.choice(
            binary_mask.size, size=int(num_pixels), replace=False
        )
    ...
    random_tensor = (
        torch.tensor(random_masked_sample).unsqueeze(0).unsqueeze(0).float().to(device)
    )
    with torch.no_grad():
        random_output = model(random_tensor)
        random_probs = (
            torch.nn.functional.softmax(random_output, dim=1).cpu().numpy()[0]
        )
