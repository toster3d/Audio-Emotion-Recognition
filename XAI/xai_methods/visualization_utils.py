...
def plot_sample_with_explanation(sample, explanation, method='gradcam', title=None):
    ...
    if method == 'gradcam':
        ...
        lrp_map = ax.imshow(explanation, cmap='seismic', alpha=0.6, vmin=vmin, vmax=vmax)
        ...
        plt.colorbar(silence_map, ax=ax, label='Prawdopodobieństwo ciszy', fraction=0.046, pad=0.04)
...
def display_xai_comparison(sample, xai_results, emotion, pred_emotion=None, figsize=(25, 6)):
    """
    ...
    if 'gradcam' in xai_results:
        ...
        # Nałożenie mapy ciepła GradCAM
        im = axs[idx].imshow(gradcam_map, cmap='jet', alpha=0.7)
        axs[idx].set_axis_off()
        axs[idx].set_facecolor('black')
        idx += 1
    ...
    if 'lrp' in xai_results:
        ...
        axs[idx].imshow(sample_disp, cmap=spectro_cmap())
        # Nałożenie mapy istotności LRP
        im = axs[idx].imshow(lrp_map, cmap='seismic', alpha=0.7, 
                         vmin=-np.max(np.abs(lrp_map)), vmax=np.max(np.abs(lrp_map)))
        axs[idx].set_axis_off()
        axs[idx].set_facecolor('black')
        idx += 1
...
def plot_verification_results(results, title="Weryfikacja metody XAI"):
    ...
    important_drops = [r.get('prob_drop_important', r.get('prob_drop_silence', 0)) for r in results]
    ...
    rects1 = ax.bar(x - width/2, important_drops, width, label='Ważne obszary')
    rects2 = ax.bar(x + width/2, random_drops, width, label='Losowe obszary')
